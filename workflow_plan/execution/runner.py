"""`StepRunner` — the boundary between a Plan and whatever performs it.
    ...
    Plan / PlanStep / Variable / Strategy      what to do, and how it is implemented
            |
            v
        StepRunner                         Protocol: one PlanStep, in and out
            |
            +-- SequentialRunner                here. In-process, sequential, no retries.
            +-- (Temporal, PydanticGraph, LangGraph)   NOT BUILT. See below.

## Protocol, not a base class

Nothing has to inherit from anything. `SequentialRunner` satisfies this by having the method, and so
...
it forces every implementation of every PlanStep to be a coroutine, including `lambda x: x + 1`.
...
is not a result. `check_strategy` reports it and `SequentialRunner` refuses it.
...
class StepRunner(Protocol):
    """Perform one PlanStep with its inputs already gathered, and return its outputs by name.

    `inputs` and the return value are both keyed by Variable NAME, so a runner never has to know
    what a Plan is — only what a PlanStep declares. That is what keeps `execute()` (which does know
    about Plans) separable from the mechanics of running one PlanStep.
    """
