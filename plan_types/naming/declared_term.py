...
conflated — `plan_types.naming.records` is still the surface that reads unannotated models.

## Why it lives here and not in `conceptlint`

It was `conceptlint.core.concept.Concept`. `conceptlint.core.lint` imports `plan_types`, so a base
in `conceptlint` that `plan_types.plan` had to inherit would close an import cycle. The direction
